...
import { NodeInstance, SocketDef } from '@/types';
...
const ReactFlowCustomNodeComponent = memo(({ data, id, selected }: CustomNodeProps) => {
  ...
  const leftHandlesRef = useRef<HTMLDivElement>(null);
  const rightHandlesRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div className={`custom-node ${selected ? 'selected' : ''} ${!nodeData.isVisible ? 'hidden' : ''}`}>
      {/* These containers are the portals for all handles */}
      <div className="handles-container-left" ref={leftHandlesRef} />
      <div className="handles-container-right" ref={rightHandlesRef} />
      
      {/* The visible content of the node */}
      <div className="node-content-wrapper">
        <HandlePortalContext.Provider value={{ left: leftHandlesRef.current, right: rightHandlesRef.current }}>
            {/* Main execution flow sockets - positioned at top */}
            {mainExecInputs.map((socket: SocketDef) => (
                <SocketHandle key={socket.id} socket={socket} isMainExec={true} isStatic={true} staticTop={37} />
            ))}
            ...
            {collapsedSockets.map(([name, sockets], index) => {
                ...
                <React.Fragment key={`${nodeData.id}-${name}`}>
                    {sockets.input && <SocketHandle key={sockets.input.id} socket={sockets.input} isStatic={true} staticTop={topPosition} />}
                    {sockets.output && <SocketHandle key={sockets.output.id} socket={sockets.output} isStatic={true} staticTop={topPosition} />}
